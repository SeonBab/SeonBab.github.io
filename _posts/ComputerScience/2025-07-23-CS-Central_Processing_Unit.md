...
워드(Word)는 CPU가 한 번에 처리할 수 있는 데이터의 기본 단위로, 일반적으로 레지스터의 크기와 동일합니다.  
즉, 32비트 CPU에서는 32비트 레지스터가 1워드, 64비트 CPU에서는 64비트 레지스터가 1워드입니다.

워드는 CPU 연산, 메모리 접근, 포인터 크기 등과 밀접하게 관련되어 있으며, 프로그램 실행 효율에도 영향을 줍니다.
