...
생성자(Constructor)는 클래스의 인스턴스가 생성될 때 자동으로 호출되는 함수입니다.
...
인스턴스 생성시 생성자가 호출 되기 전에 메모리가 할당되고 사용자가 호출하는 생성자가 작동됩니다.

생성자는 클래스와 구조체에서 사용됩니다.

함수와 다르게 생성자의 정의에는 다음과 같은 규칙이 있습니다.

+ 생성자의 접근제한자는 특수한 경우를 제외하고 `public`으로 선언되어야 합니다.  
- 생성자의 이름은 클래스 이름과 동일해야하고, 반환값을 가지지 않습니다.  
