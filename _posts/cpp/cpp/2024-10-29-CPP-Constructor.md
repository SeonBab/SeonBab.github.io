...
+ 생성자 또한 접근제한자의 영향을 받습니다.
    - `protected`일 경우 상속 관계에서만 생성자가 호출될 수 있습니다.
+ 생성자의 이름은 클래스 이름과 동일해야합니다.
+ 생성자는 반환 타입이 없고, 명시하지도 않아 void를 지정하지도 않습니다.
    - 따라서 값을 반환하지도 않습니다.
...
정의된 생성자가 없다면 컴파일러가 자동으로 기본 생성자를 만듭니다.

생성자 본문이 실행되는 시점에는 멤버 변수들이 이미 생성된 상태입니다.  
따라서 생성자 내부에서 값을 설정하는 것은 초기화가 아니라, 이미 생성된 객체에 값을 대입(할당)하는 과정입니다.  
이러한 이유로 `const` 혹은 참조는 생성자 본문에서 초기화를 할 수 없기 때문에 멤버 변수를 생성과 동시에 초기화가 필요할 경우 생성자 멤버 초기화 리스트를 사용하는 것이 좋습니다.

기본 자료형인 멤버 변수는 초기화되지 않기 때문에 쓰레기값을 가지고 있습니다.
...
매개 변수를 받는 생성자(Parameterized Constructor)는 사용자가 특정한 값을 전달하여 멤버 변수의 값을 초기화할 수 있도록 하는 생성자입니다.  
기본 생성자와 다르게 원하는 값으로 멤버 변수의 값을 초기화 할 수 있습니다.

생성자를 하나라도 직접 정의하면 컴파일러는 기본 생성자를 자동으로 생성하지 않으므로, 기본 생선자가 필요할 경우 명시적으로 선언해야합니다.
...
위의 예시와 같이 생성자 본문에서 매개변수 값을 대입하는 경우 이는 초기화가 아닌 할당이 이루어집니다.  
이 경우 멤버 변수는 먼저 기본 초기화된 이후 값이 대입되므로 비효율이 발생할 수 있으며, `const`멤버나 참조형 멤버와 같이 생성 시점에 반드시 초기화 되어야 하는 변수는 이러한 방식으로 처리할 수 없습니다.  
이러한 문제를 해결하기 위해 생성자 멤버 초기화 리스트를 사용할 수 있습니다.

## 생성자 멤버 초기화 리스트

생성자 멤버 초기화 리스트(Constructor Member Initializer List)는 생성자 함수 내부에서 대입 연산자(=)를 사용하는 것보다 효율적으로 멤버 변수를 초기화할 수 있습니다.  
기본 생성자와 매개 변수를 받는 생성자에서는 대입 연산자를 사용했는데, 이 경우 이미 생성된 객체에 값을 할당하는 것이고, 멤버 초기화 리스트는 객체 생성과 동시에 초기화를 수행합니다.
...
이 외에도 멤버 초기화 리스트는 초기화 하는 순서가 굉장히 중요합니다.  
순서에 따라 문제가 발생할 수 있기 때문입니다.
...
{
public:
    ConstExample(int value, int& r) : a(value), ref(r), sum(a + ref)
    {
        std::cout << "a: " << a << ", ref: " << ref << ", sum: " << sum << std::endl;
    }
...
결과부터 이야기 하자면 이 코드는 잘 못 된 멤버 변수 순서로 인해 `Sum(a + ref)`에서 정의 되지 않은 동작(UB/Undefined Behavior)가 발생합니다.

그 이유는, 초기화 순서가 생각한 것과 다르게 동작하기 때문입니다.  
순서에 대해 제대로 이해하기 전에는 일반적으로 다음과 같이 생각합니다.

1. `a` 초기화
2. `ref` 초기화
3. `sum` 초기화

하지만, 멤버 초기화 리스트는 작성된 순서가 아니라 멤버 변수의 선언 순서에 따라 초기화됩니다.

위 클래스의 멤버 선언 순서에 따라 초기화 되며, 선언 순서는 다음과 같습니다.

1. `sum`
2. `a`
3. `ref`

따라서 실제 초기화는 다음 순서로 이루어집니다.

1. `sum` 초기화
2. `a` 초기화
3. `ref` 초기화

이때 `sum`을 초기화하는 시점에는 `a`와 `ref`가 아직 초기화되지 않은 상태이므로, 초기화 되지 않은 값을 참조하게 됩니다.  
그 결과, 정의되지 않은 동작(Undefined Behavior)이 발생할 수 있습니다.
