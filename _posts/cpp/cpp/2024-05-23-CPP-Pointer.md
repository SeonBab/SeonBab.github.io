...
`NULL`은 매크로로 정의되며, C++에서는 보통 `0`으로 정의된다.  
포인터에 `0`을 대입하면 이는 null pointer value(아무 객체도 가리키지 않는 상태)를 의미합니다.

하지만 0은 본래 `int`타입의 정수 리터럴이며, 특별한 규칙에 의해 포인터로 변환될 수 있는 것일 뿐입니다.  
이로 인해 함수 오버로딩 등에서 모호성이 발생할 수 있습니다.  
이러한 문제를 해결하기 위해 C++11부터는 명확한 null pointer 표현인 `nullptr`(std::nullptr_t) 사용이 권장됩니다.

또한 `(void*)0`, `0`, `nullptr`은 서로 다른 타입과 의미를 가집니다.  
`NULL`은 고유한 타입이 아니라 구현에 따라 정의되는 매크로입니다.

### 잘못 초기화하는 예시

포인터는 잘못 사용할 경우 심각한 오류를 발생시킬 수 있습니다.  
아래 코드를 보고 어떤 문제가 발생하는지 생각해봅시다.
...
이 코드는 컴파일은 정상적으로 되지만, 실행 시 런타임 에러가 발생할 수 있습니다.  
`pChar`가 아무것도 가리키지 않는데, 존재하지 않는 메모리를 접근하려고 하기 때문에 null pointer를 역참조 한것이 됩니다.
...
해당 코드는 주소를 저장해야하는데 그냥 값을 넣으려고 하기 때문에 컴파일 에러가 발생합니다.
