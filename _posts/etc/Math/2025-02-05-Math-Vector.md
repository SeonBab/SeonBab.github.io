...
물리적 벡터(Vector)는 크기와 방향을 가진 물리량이나 수학적 대상을 의미합니다.  
수학적 벡터는 벡터 공간의 원소를 의미합니다.
...
벡터와 벡터의 덧셈 및 뺄셈과 벡터와 스칼라의 곱셈은 벡터의 생성 시스템에 사용하는 선형 조합을 구성하는 필수 연산입니다.
...
뺄셈은 단순히 성분을 빼는 연산이 아니라, 벡터 $b$의 끝점에서 벡터 $a$의 끝점을 향하는 방향 벡터를 의미합니다.

이것으로 두 점 사이의 방향을 구할 때 사용됩니다.

예를 들어, 플레이어 위치를 $P$, 적의 위치를 $E$라고 할 때 $E - P$는 플레이어에서 적을 향하는 방향 벡터가 됩니다.
...
주어진 벡터들을 스칼라로 곱해서 더하는 연산 자체를 의미합니다.

$a_1v_1 + a_2v_2 + a_3v_3 + \cdots + a_nv_n = v$

평면의 경우, 평행하지 않은 두 기저벡터를 선형조합하면 평면상의 모든 벡터를 생성할 수 있습니다.
...
첫 번째 식에 2를 곱하면 $12 = 2a + 4b$가 됩니다.  
하지만 두 번째 식에서는 $7 = 2a + 4b$이므로, $12 = 7$이라는 모순이 발생합니다.

따라서 이 연립방정식은 해를 가지지 않으며, 이 두 벡터로는 $(6, 7)$을 생성할 수 없습니다.

또한 위 식에서 항상 $y = 2x$ 관계가 성립하므로, 이 두 벡터로는 $y = 2x$ 직선 위의 점들만 생성할 수 있습니다.
...
일반적인 수학적 정의는 다음과 같습니다.

$c_2v_1 + c_2v_2 + \cdots + c_nv_n = 0$일 때,  
$c_1 = c_2 = \cdots = c_n = 0$인 경우에만 성립하면, 이 벡터들은 선형 독립입니다.
...
기저는 벡터 공간의 모든 벡터를 생성할 수 있는 독립적인 벡터 집합입니다.
...
표준기저벡터(Standard Basis Vector)는 평면의 모든 벡터를 생성할 수 있는 기저(Basis)에서 가장 기본적인 기저 벡터입니다.
